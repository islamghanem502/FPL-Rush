...
const PrivateChallengeFormPage = () => {
  ...
  const submit = (payload) => {
    ...
    const callbacks = {
      onSuccess: (response) => {
        ...
        setCreatedInvite({ inviteCode: data.inviteCode, inviteUrl: data.inviteUrl });
      },
  ...
  if (isEdit && challenge && !challenge.isOwner) {
    return <Layout><div className="max-w-xl mx-auto p-10 text-center text-white">لا تملك صلاحية تعديل هذا التحدي.</div></Layout>;
  }
    ...
    <Layout>
      <main className="max-w-3xl mx-auto px-4 py-10" dir="rtl">
        ...
        <section className="mt-5 bg-slate-900/70 border border-slate-700 rounded-3xl p-6 sm:p-8 shadow-2xl">
          ...
          <p className="text-gray-400 text-sm mb-8">لن يظهر هذا التحدي للعامة. بعد الحفظ ستأخذ رابطًا وكودًا لمشاركته مع من تريد.</p>
          {error && <div className="mb-5 rounded-xl bg-red-500/10 border border-red-500/30 text-red-300 p-3 text-sm font-bold">{error}</div>}
          <ChallengeForm initialValue={initialValue} onSubmit={submit} isSaving={createMutation.isPending || updateMutation.isPending} submitLabel={isEdit ? 'حفظ التعديلات' : 'إنشاء التحدي والرابط'} />
        </section>
        ...
        {invite && (
          <section className="mt-6 bg-[#22c55e]/10 border border-[#22c55e]/40 rounded-3xl p-6 text-right">
            ...
            <p className="text-gray-300 text-sm mt-2">شارك الرابط أو الكود مع المشاركين. لا يظهران في أي قائمة عامة.</p>
            ...
            <div className="mt-3 text-center text-white font-black tracking-widest">{invite.inviteCode}</div>
